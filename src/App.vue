    ...
    <div class="h-screen col-span-10 grid grid-cols-5 bg-white">
      <div class="col-span-2 py-16 ">
        <div class="flex">
          ...
          <h1 class="title text-7xl font-extrabold p-3"> {{ pomodoroData.focusTime }}</h1>
        </div>
      ...
      <div class="cornometres col-span-3 flex relative">
        <pomodoroItem :pomodoroTime="pomodoroData.focusTime" :timerClass="focusClass" @click="breakClicked = 1"
          :class="{ 'absolute z-30': breakClicked === 1, 'mx-auto z-20': breakClicked !== 1 }"></pomodoroItem>
        <pomodoroItem :pomodoroTime="pomodoroData.shortBreakTime" :timerClass="shortBreakClass" @click="breakClicked = 2"
          :class="{ 'mx-auto z-20': breakClicked === 1, 'absolute z-30': breakClicked === 2, 'absolute end-0 z-1': breakClicked === 3 }">
        </pomodoroItem>
        <pomodoroItem :pomodoroTime="pomodoroData.longBreakTime" :timerClass="longBreakClass" @click="breakClicked = 3"
          :class="{ 'absolute end-0 z-1': breakClicked !== 3, 'absolute z-30': breakClicked === 3 }"></pomodoroItem>
      </div>

    </div>

    <modalSeting v-for="(item, index) in modalColor" :key="index" :color="item" @allValue="handaleValue"
...
export default {
  ...
  data() {
    return {
      ...
      pomodoroData: {},
      modalColor: [
        { bgColor: 'focus-color', btnColor: '#471515' },
        { bgColor: 'short-break-color', btnColor: '#14401D' },
        { bgColor: 'long-break-color', btnColor: '#153047' },
      ]
    }
  },
  methods: {
